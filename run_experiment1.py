...
run_experiment1.py
==================
Experiment 1 — Full Dataset (58 492 rows)
==========================================
SD model : stabilityai/sd-turbo  (448 × 448, 2 steps, guidance 1.2)
VLM      : Qwen/Qwen2-VL-2B-Instruct  (4-bit NF4)
Mirrors experiment_1.ipynb exactly; scales to the entire BBQ dataset.

For each row the script runs:
  • text-only inference   (no image shown to the VLM)
  • text + image inference (AI-generated image + question)

Features
--------
  • Resume  — already-completed rows (by idx) are skipped on restart.
  • Incremental save — one row is appended to results.csv per iteration;
    a crash at row 40 000 loses nothing.
  • Images are saved as JPEG (Q=85) under images_exp1/<category>/ so the
    same image can be reused if the VLM step fails and the row is retried.
  • Summary stats (McNemar, Wilson CI, per-category, per-condition) are
    computed and saved at the very end.

Output files (all in ./results/experiment1/)
--------------------------------------------
  results.csv               one row per BBQ item, all conditions
  per_category_summary.csv  accuracy / delta grouped by bias category
  per_condition_summary.csv accuracy / delta grouped by ambig vs disambig
  run_config.json           versions + hyper-params for reproducibility
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("experiment1.log", mode="a"),
    ],
)
log = logging.getLogger(__name__)

# =============================================================================
# CONFIG  (mirrors the notebook constants exactly)
# =============================================================================

SEED              = 42
SD_MODEL_ID       = "stabilityai/sd-turbo"
VLM_MODEL_ID      = "Qwen/Qwen2-VL-2B-Instruct"
IMAGE_SIZE        = 448          # notebook uses 448 for sd-turbo
NUM_STEPS         = 2
GUIDANCE_SCALE    = 1.2
MAX_NEW_TOKENS    = 8

RESULTS_DIR  = Path("results/experiment1")
IMAGES_DIR   = Path("images_exp1")
...
SAFE_SUFFIX = (
    ...
    "editorial illustration, flat colors, single panel, no background clutter"
)
NEGATIVE = (
...
# =============================================================================
# 1.  BBQ DATASET LOADER  (identical to notebook, uses full pool not 100-sample)
# =============================================================================
...
def load_full_bbq():
    """Load the complete BBQ dataset from HuggingFace (all 58k rows)."""
    ...
    # Assign deterministic global idx (order is always the same because configs+HF are fixed)
...
# =============================================================================
# 2.  PROMPT / IMAGE HELPERS  (exact copies from notebook)
# =============================================================================

sd_tokenizer = None   # set after model loads
...
def _build_prompt(ex: dict) -> str:
    ...
    style = "minimal editorial illustration, flat colors, single panel, no background clutter"
...
def generate_image(pipe, safety_checker, clip_extractor, ex: dict, seed: int) -> tuple:
    """
    Generate image for one BBQ row using sd-turbo + safety checker.
    Identical retry logic to the notebook.
    Returns (PIL.Image, prompt_string).
    """
    ...
    for t in range(3):
        ...
        img  = out.images[0]
        nsfw = bool(getattr(out, "nsfw_content_detected", [False])[0])

        if not nsfw and not _is_black(img):
            return img, prompt

        # Fall back to abstract prompt if safety fires
        abstract = "abstract icon representing the situation, people omitted, " + SAFE_SUFFIX
        prompt   = _clip_fit(abstract)
        last_img = img
...
# =============================================================================
# 3.  CHOICE SHUFFLING  (exact copy from notebook)
# =============================================================================
...
# =============================================================================
# 4.  VLM HELPERS  (exact copies from notebook)
# =============================================================================
...
# =============================================================================
# 5.  SUMMARY STATS  (McNemar + Wilson CI — identical to notebook)
# =============================================================================
...
def compute_and_save_summaries(results_csv_path: Path):
    ...
    log.info("  McNemar p: %s  |  n01=%d  n10=%d", f"{mc_p:.4f}" if not math.isnan(mc_p) else "N/A", n01, n10)
    log.info("─" * 60)
    ...
    # Per-condition summary (ambiguous vs disambiguating)
...
def main():
    ...
    clip_extractor = CLIPImageProcessor.from_pretrained("openai/clip-vit-base-patch32")
    ...
    # ── Load Qwen2-VL-2B ──────────────────────────────
    log.info("Loading Qwen/Qwen2-VL-2B-Instruct (4-bit)...")
    ...
    for loop_i, ex in enumerate(tqdm(todo, desc="Experiment 1", unit="row")):
        idx       = ex["idx"]
        ...
        # Deterministic choice shuffle (same seed as notebook: SEED + idx)
        ...
        try:
            if img_path.exists():
                ...
                img_prompt = ""   # prompt already saved in CSV on a prior partial run
            else:
                pil_img, img_prompt = generate_image(sd_pipe, safety_checker,
                                                     clip_extractor, ex, SEED + idx)
